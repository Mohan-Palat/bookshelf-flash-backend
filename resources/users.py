...
def login():
    ...
    try:
        user = models.User.get(models.User.email== payload['email']) ### Try find the user by their email
        ...
        if(user_dict['password'], payload['password']): # use bcyrpts check password to see if passwords match
